...
function generateBoard(currentBoard) {
    ...
    for (let i = 0; i < currentBoard.length; i++) {
        ...
        for (let j = 0; j < currentBoard[i].length; j++) {
            ...
            if(isNeighborFromTopFound(currentBoard, i, j)) {
                neighbors++;
            }

            if(isNeighborFromTopRigthFound(currentBoard, i, j)) {
                neighbors++;
            }

            if(isNeighborFromRigthFound(currentBoard, i, j)) {
                neighbors++;
            }

            if(isNeighborFromBottomRigthFound(currentBoard, i, j)) {
                neighbors++;
            }

            if(isNeighborFromBottomFound(currentBoard, i, j)) {
                neighbors++;
            }

            if(isNeighborFromBottomLeftFound(currentBoard, i, j)) {
                neighbors++;
            }

            if(isNeighborFromLeftFound(currentBoard, i, j)) {
                neighbors++;
            }

            if(isNeighborFromTopLeftFound(currentBoard, i, j)) {
                neighbors++;
            }
